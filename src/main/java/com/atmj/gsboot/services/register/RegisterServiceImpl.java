...
import com.atmj.gsboot.dbaccess.repositories.RegisterRepository;
...
public class RegisterServiceImpl implements RegisterService {
	 ...
	 *            List<InOut> inout = new ArrayList<InOut>(); Iterator<UserEntity>
	 *            ilin = lin.iterator(); InEntity in; Long id; while
	 *            (ilin.hasNext()) { id = ilin.next().getIdemployee(); if (id !=
	 *            null) { in = new InEntity(); in.setDate(new DateUtil().getNow());
	 *            in.setEmployee(employeesRepository.findById(id));
	 *            inRepository.save(in); inout.add(mapper.map(in, InOut.class)); } }
	 * 
	 *            }
	 * 
	 *            public List<Register> out(List<UserEntity> lout) { List<Register>
	 *            inout = new ArrayList<Register>(); Iterator<UserEntity> ilin =
	 *            lout.iterator(); OutEntity in; while (ilin.hasNext()) { in = new
	 *            OutEntity(); in.setOutdate(new Date());
	 *            in.setEmployee(mapper.map(ilin.next(), UserEntity.class));
	 *            outRepository.save(in); inout.add(mapper.map(in, Register.class));
	 *            } return inout; }
	 **/
	...
	public List<RegisterEntity> findByDates(String datefrom, String dateuntil) {
		 ...
		 * employeesRepository.findByEnabledTrue(); // Iterator<UserEntity> iemployees =
		 * employees.iterator();
		 **/
	...
	/**
	 * private void disabledRegister(UserEntity employee, List<Date> dates) { Date
	 * date; RegisterEntity register; Iterator<Date> idates = dates.iterator();
